...
def train_epoch_adv(generator, discriminator, train_loader, optimizers, cfg):
    ...
    for i, (x, _) in enumerate(train_loader):
        ...
        if logger:
            ...
            logger.add_scalar('loss_pack_fake_iter', loss_pack_fake_val, num_iter_global)
        ...
        loss_pack_fake_value += loss_pack_fake_val * batch_size
        loss_d_value += loss_d_val * batch_size
        loss_g_value += loss_g_val * batch_size

        num_iter_global += 1

    loss_pack_fake_value /= len(train_loader.dataset)
    loss_d_value /= len(train_loader.dataset)
    loss_g_value /= len(train_loader.dataset)

    return loss_pack_fake_value, loss_d_value, loss_g_value
...
def train(args=None):
    ...
    for epoch in range(1, cfg['num_epoch'] + 1):
        ...
        if logger:
            logger.add_scalar('loss_pack_epoch', loss_pack, epoch)
...
def parse():
    ...
    args.cfg = 'cfgs/dcgan/dcgan_asymmetric_one_stage_celeba.yml'
